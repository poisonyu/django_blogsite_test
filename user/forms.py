...
class RegForm(forms.Form):
    ...
    def clean(self):
        ...
        if not (verification_code != '' and verification_code == code):
...
class BindEmailForm(forms.Form):
    ...
    def clean(self):
        ...
        if self.request.user.email != '':
            raise forms.ValidationError('邮箱已经绑定')
        
        # 判断验证码
        code = self.request.session.get('bind_email_code', '')
        verification_code = self.cleaned_data.get('verification_code', '')
        if not (verification_code != '' and code == verification_code):
...
class ChangePasswordForm(forms.Form):
    ...
    def clean_old_password(self):
        # 验证旧密码是否正确
        old_passwrod = self.cleaned_data.get('old_password', '')
        if not self.user.check_password(old_passwrod):
